...
class Pairwise(Dataset):
    ...
    def _crop_and_resize(self, image, box):
        # convert box to 0-indexed and center 详情见exempler_&&_instance.ipython和test_module_for_testpy的_crop_and_resize
        box = np.array([
            box[0] - 1 + (box[2] - 1) / 2,
            box[1] - 1 + (box[3] - 1) / 2,
            box[2], box[3]], dtype=np.float32)
        center, target_sz = box[:2], box[2:]   #返回框框中心点和w，h

        # exemplar and search sizes
        context = self.cfg.context * np.sum(target_sz)#p/2
        z_sz = np.sqrt(np.prod(target_sz + context))      #   paper上的s              #prob元素乘积
        ...
        # crop image patch
        corners = tuple((corners + npad).astype(int))#2个角坐标
